...
pub fn apply_theme(ctx: &egui::Context, theme: crate::config::Theme) {
    ...
    match theme {
        ...
        crate::config::Theme::Light => {
            ...
            vis.widgets.noninteractive.bg_stroke = egui::Stroke::new(
                1.0,
                egui::Color32::from_rgb(0xC8, 0xCD, 0xD6),
            );
            vis.widgets.noninteractive.bg_fill = egui::Color32::from_rgb(0xF6, 0xF8, 0xFB);
            vis.widgets.inactive.fg_stroke =
                egui::Stroke::new(1.0, egui::Color32::from_rgb(0x1A, 0x1D, 0x24));
            // Checkbox / 控件边框：白底 / chrome 上都看得清，比 egui 默认更显眼。
            vis.widgets.inactive.bg_stroke = egui::Stroke::new(
                1.0,
                egui::Color32::from_rgb(0x6E, 0x74, 0x80),
            );
            // Checkbox 空白底：略灰，避免与 window_fill / extreme_bg_color 完全融为一体。
            vis.widgets.inactive.bg_fill = egui::Color32::from_rgb(0xD6, 0xDD, 0xE6);
            vis.widgets.hovered.bg_stroke = egui::Stroke::new(
                1.0,
                egui::Color32::from_rgb(0x4F, 0x8C, 0xFF),
            );
            vis.widgets.hovered.bg_fill = egui::Color32::from_rgb(0xDC, 0xE4, 0xF2);
        }
    }
